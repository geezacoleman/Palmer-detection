    ...
    def predict_model(self, model, dataloader, task):
        ...
        if self.do_pr_metric:
            ...
            if len(stats) and stats[0].any():
                ...
                if self.v5_metric:
                    p, r, ap, f1, ap_class = ap_per_class(*stats, v5_metric=self.v5_metric, plot=self.plot_curve,
                                                          save_dir=self.save_dir, names=model.names)
                    ...
                    p, r, ap, f1, ap_class = ap_per_class(*stats, plot=self.plot_curve,
                                                          save_dir=self.save_dir, names=model.names, v5_metric=self.v5_metric)
                    ap50, ap = ap[:, 0], ap.mean(1)  # AP@0.5, AP@0.5:0.95
                ...
                if self.save_metrics:
                    metrics_for_txt.append([os.path.basename(self.save_dir), 'all', seen, nt.sum(), mp, mr, map50, map])
                ...
                if self.verbose and model.nc > 1:
                    for i, c in enumerate(ap_class):
                        if self.v5_metric:
                            LOGGER.info(pf % (model.names[c], seen, nt[c], p[i], r[i], ap50[i], ap[i]))
                        else:
                            LOGGER.info(pf % (model.names[c], seen, nt[c], p[i, AP50_F1_max_idx], r[i, AP50_F1_max_idx],
                                               f1[i, AP50_F1_max_idx], ap50[i], ap[i]))
                        if self.save_metrics:
                            metrics_for_txt.append([os.path.basename(self.save_dir), model.names[c], seen, nt[c], p[i], r[i], ap50[i], ap[i]])
            ...
            if self.save_metrics:
                print('COMPLETE: Saving metrics')
                import csv
                metrics_output_path = os.path.join(self.save_dir, "metrics.csv")
                with open(metrics_output_path, "w") as f:
                    writer = csv.writer(f)
                    for class_metric_list in metrics_for_txt:
                        writer.writerow(class_metric_list)
                print("Metrics saved to {}".format(metrics_output_path))
