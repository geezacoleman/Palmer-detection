...
class DetectionValidator(BaseValidator):
    ...
    def print_results(self):
        ...
        metrics_for_txt.append([os.path.basename(self.save_dir), 'all', self.seen, self.nt_per_class.sum(),
                                self.metrics.mean_results()[0], self.metrics.mean_results()[1],
                                self.metrics.mean_results()[2], self.metrics.mean_results()[3]])

        # Print results per class
        if (self.args.verbose or not self.training) and self.nc > 1 and len(self.stats):
            for i, c in enumerate(self.metrics.ap_class_index):
                self.logger.info(pf % (self.names[c], self.seen, self.nt_per_class[c], *self.metrics.class_result(i)))
                metrics_for_txt.append([os.path.basename(self.save_dir), self.names[c], self.seen, self.nt_per_class[c],
                                        self.metrics.class_result(i)[0], self.metrics.class_result(i)[1],
                                        self.metrics.class_result(i)[2], self.metrics.class_result(i)[3]])
        ...
        self.logger.info(f"Metrics saved to {colorstr('bold', metrics_output_path)}")
