...
def test(data,
         ...
         save_metrics=False,  # ADDED - saves output metrics to CSV for analysis
         plots=True,
         wandb_logger=None,
         compute_loss=None,
         half_precision=True,
    ...
    if save_metrics:
        ...
        print(metrics_for_txt)

    # Print results
    pf = '%20s' + '%12i' * 2 + '%12.3g' * 4  # print format
    if save_metrics:
        metrics_for_txt.append([opt.name, 'all', seen, nt.sum(), mp, mr, map50, map])
    ...
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
        for i, c in enumerate(ap_class):
            ...
            if save_metrics:
                metrics_for_txt.append([opt.name, names[c], seen, nt[c], p[i], r[i], ap50[i], ap[i]])
